...
class Planet{
	...
	//the "this" pointer should be a pointer that holds the address of the Planet object (this class).
		long int id;  //If we are using "this" for the address which is already done in the constructor, then we probably don't need this variable.
    ...
    int orbit();
		long int getID() {return (long int) this->id;} //This needs to be a long int to hold the address -- apparently the address can be stored in a variable that is not a pointer?
